...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(1), __webpack_require__(7), __webpack_require__(42), __webpack_require__(12), __webpack_require__(22), __webpack_require__(16), __webpack_require__(25), __webpack_require__(4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (require, exports, core_1, html_1, _data_1, events_1, Forms_1, calendar_1, FormValid_1, array_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var selectAll = html_1.HTML.selectAll;
    var propertyMap = events_1.Events.propertyMap;
    var isodate = calendar_1.Calendar.isodate;
    core_1.$ready(function () {
        var $section = '정치 경제 사회 건강 국제 교육 기타'.split(' ');
        // 먼저 모든 데이터를 불러온다.
        _data_1.Data.list().then(function (dataMap) {
            selectAll(document.body, ['.news-list', '.subject', '.form', '.sections',
                '#search', '#select-check', '#save', '#id', '.section'], function (body, news, subject, _form, sections, search, selectCheck, saveBtn, idInput, section) {
                // 섹션 선택 라디오버튼 만들기
                section.innerHTML = $section.map(function (p, i) {
                    var c = 'section' + i;
                    return '<input type="radio" id="' + c + '" name="section" value="' + p + '">' +
                        '<label for="' + c + '">' + p + '</label>';
                }).join('');
                var classList = _form.classList, form = Forms_1.Forms.createForms(_form), formValid = function (e) {
                    if (e === false) {
                        classList.remove('on');
                    }
                    else {
                        if (FormValid_1.FormValid.$valid(form))
                            classList.add('on');
                        else
                            classList.remove('on');
                    }
                }, active, list = function (values) {
                    if (values === void 0) { values = []; }
                    subject.innerHTML = values.sort(function (a, b) {
                ...
                // 새로고침시 사용
                subjectRender = function () { return list(); }, 
                // 신문사 종류
                $refresh = function () {
                    news.innerHTML = dataMap.names.map(function (v) {
                        return '<span class="menu" data-event="news" data-value="name:' + v + '">' + v + "</span>";
                    }).join('');
                    subjectRender();
                    // 섹션계산
                    var obj = array_1._reduceN($section, function (r, v) { return r[v] = 0; }, {});
                    dataMap.each(function (v) { return obj[v.section]++; });
                    sections.innerHTML = $section.map(function (s) {
                ...
                propertyMap(body, 'click', {
                    news: function (_a) {
                        var name = _a.name;
                        subjectRender = function () { return list(dataMap.newsMap[name]); };
                        subjectRender();
                    },
                    subject: function (_a) {
                        var id = _a.id;
                        var data = dataMap.idMap[id];
                        formValid(false);
                        form.reset(data);
                    },
                    create: function () {
                    ...
                    section: function (_a) {
                        ...
                        subjectRender = function () { return list(dataMap.filter(function (v) { return v.section === section; })); };
                        subjectRender();
                    }
                });
                // 검색
                search.addEventListener('keyup', function () {
                    ...
                    if (value) {
                        var reg_1 = new RegExp(value.trim(), 'g');
                        list(dataMap.filter(function (v, p) { return reg_1.test(v.subject); }));
                    }
                    else
                        list();
                });
                ...
                selectCheck.addEventListener('click', function () {
                    subjectRender = function () { return list(dataMap.filter(function (v, p) { return !!v.chk; })); };
                    subjectRender();
                });
                // 저장
                saveBtn.addEventListener('click', function () {
                    var d = form.values(), id = d.id, datetime = d.datetime;
                    d.datetime = isodate(datetime);
                    if (id) {
                        _data_1.Data.update(d).then(function () {
                            d.datetime = datetime;
                        ...
                        _data_1.Data.save(d).then(function (id) {
                            d.id = id;
                            d.datetime = datetime;
                            dataMap.add(new _data_1.Data(d));
                            $refresh();
                            alert('저장');
                        });
                    }
                ...
                section.addEventListener('change', function (e) {
                    e.stopPropagation();
                    var i = idInput.value, v = e.target['value'];
                    _data_1.Data.section(i, v).then(function () {
                        dataMap.update(i, function (u) { return u.section = v; });
                        $refresh();
                    });
                });
                $refresh();
...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(1), __webpack_require__(4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (require, exports, core_1, array_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var $assemble = {
    ...
    var DataMap = /** @class */ (function () {
        ...
        DataMap.prototype.update = function (id, h) {
            h(this.idMap[id]);
            return this;
        };
        DataMap.prototype.filter = function (h) {
            var p, newsMap = this.newsMap, array = [];
            for (p in newsMap) {
                newsMap[p].forEach(function (v) { return h(v, p) === true && array.push(v); });
            }
            return array;
        };
        DataMap.prototype.each = function (h) {
            var p, newsMap = this.newsMap;
            for (p in newsMap) {
                newsMap[p].forEach(function (v) { return h(v, p); });
            }
            return this;
        };
        DataMap.prototype.add = function (data) {
            var _a = this, newsMap = _a.newsMap, names = _a.names, idMap = _a.idMap, id = data.id, news = data.news, array = newsMap[news];
            if (!array) {
                names.push(news);
                array = newsMap[news] = [];
            }
        ...
        DataMap.prototype.remove = function (data) {
            var array = this.newsMap[data.news];
    ...
    var Data = /** @class */ (function () {
        ...
        Data.prototype.reset = function (data) {
            core_1.$extend(this, data, $disassmble);
            return this;
        };
    ...
    (function (Data) {
        function list() {
            return $get('/nabook/list').then(function (r) {
                var dataMap = new DataMap();
                r.forEach(function (v) { return dataMap.add(new Data(v)); });
                return dataMap;
            });
        }
        Data.list = list;
        ...
        Data.update = update;
        function sections() {
            return $get('/nabook/sections').then(function (v) {
                return array_1._reduceN(v, function (r, v) { return r[v.name] = v.count; }, {});
            });
        }
